...
| `venue_events` | The events a venue can be booked for, managed per workspace | `id` | `workspace_id -> workspaces.id`; referenced by `venue_bookings.event_id` |
| `venue_bookings` | Venue booking header and stored state | `id` | `workspace_id -> workspaces.id`, `venue_id -> venues.id` (RESTRICT), `event_id -> venue_events.id` (RESTRICT); referenced by `venue_booking_slots.venue_booking_id` |
...
| `id` | `uuid` | `gen_random_uuid()` | No | Yes | Primary key. |
| `workspace_id` | `uuid` | None | No | No | Foreign key to `workspaces.id`. |
| `name` | `text` | None | No | Yes | 1–120 characters. Unique per workspace, case-insensitively and ignoring surrounding whitespace. Becomes the booking's title. |
| `description` | `text` | `null` | Yes | No | Internal description of what the event involves. |
| `active` | `boolean` | `true` | No | No | Inactive events are hidden from the public app but keep their history. |
| `sort_order` | `integer` | `0` | No | No | Display order, then name. |
| `created_at` | `timestamptz` | `now()` | No | No | |
| `updated_at` | `timestamptz` | `now()` | No | No | Maintained by the `set_updated_at` trigger. |

"Other" is deliberately not a row here. Choosing it sends no `event_id` at
all, just the submitter's own text in `venue_bookings.event_other`, so the
escape hatch cannot be renamed, deactivated or deleted out from under the
public flow.
...
| `event_id` | `uuid` | `null` | Yes | No | Foreign key to `venue_events.id`, `ON DELETE RESTRICT`: an event that has been booked cannot be deleted, only deactivated. Null when the submitter chose "Other". |
| `event_other` | `text` | `null` | Yes | No | The submitter's own description, set only when `event_id` is null. A CHECK stops both from being set at once; the submit RPC is what requires one of them. |
| `title` | `text` | None | No | No | 1–120 characters. Derived by the submit RPC from the event name, or from `event_other` trimmed to fit — never supplied by the client. |
...
- A booking names its event exactly one way: an `event_id`, or `event_other`.
  Rows that predate `venue_events` have neither, which is why the CHECK only
  forbids setting both — the "exactly one" rule lives in the submit RPC, the
  table's only writer.
