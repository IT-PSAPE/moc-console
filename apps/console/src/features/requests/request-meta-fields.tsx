...
export function RequestMetaFields({ request, editable = false, onFieldChange }: RequestMetaFieldsProps) {
  ...
  return (
    <div className="space-y-3">
      <MetaRow icon={<Loader />} label="Status">{isEditable ? <Select.Root name="request-status" items={statusItems} value={request.status} onValueChange={changeStatus}><Select.Trigger aria-label="Request status" className="w-44" style="ghost" /><Select.Content>{statuses.map(renderStatus)}</Select.Content></Select.Root> : <Badge label={statusLabel[request.status]} icon={statusIcon[request.status]} color={statusColor[request.status]} />}</MetaRow>
      <MetaRow icon={<CircleChevronDown />} label="Priority">{isEditable ? <Select.Root name="request-priority" items={priorityItems} value={request.priority} onValueChange={changePriority}><Select.Trigger aria-label="Request priority" className="w-44" style="ghost" /><Select.Content>{priorities.map(renderPriority)}</Select.Content></Select.Root> : <Badge label={priorityLabel[request.priority]} icon={<CircleAlert />} color={priorityColor[request.priority]} />}</MetaRow>
      <MetaRow icon={<Tag />} label="Type">{isEditable ? <Select.Root name="request-category" items={categoryItems} value={request.category} onValueChange={changeCategory}><Select.Trigger aria-label="Request category" className="w-44" style="ghost" /><Select.Content>{categories.map(renderCategory)}</Select.Content></Select.Root> : <Badge label={categoryLabel[request.category]} icon={<Tag />} color="purple" />}</MetaRow>
      <MetaRow icon={<Calendar />} label="Due date">{isEditable ? <DateTimeFields ariaLabel="Due date" name="due-date" value={formatUtcIsoForBrowserDateTimeInput(request.dueDate)} onChange={changeDueDate} required style="ghost" /> : <Paragraph.sm>{formatUtcIsoInBrowserTimeZone(request.dueDate)}</Paragraph.sm>}</MetaRow>
