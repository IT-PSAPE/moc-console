...
export function FormLabel({ label, required, optional, info, className }: LabelProps) {
    return (
        <span className={cn("flex justify-start items-center gap-0.5", className)}>
            <TextLabel.sm className="text-primary">{label}</TextLabel.sm>
            {required && <TextLabel.sm className="text-brand_secondary">*</TextLabel.sm>}
            {optional && <Paragraph.sm className="text-quaternary">(Option)</Paragraph.sm>}
...
export function FormField({ children, label, required, optional, info, className }: LabelProps & { children: React.ReactNode }) {
    return (
        <div className="flex flex-col gap-1.5">
            <FormLabel label={label} required={required} optional={optional} info={info} className={className} />
            {children}
        </div>
    )
}