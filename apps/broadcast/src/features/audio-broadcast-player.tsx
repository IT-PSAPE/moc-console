...
type AudioBroadcastPlayerProps = {
  broadcast: Broadcast
}

export function AudioBroadcastPlayer({ broadcast }: AudioBroadcastPlayerProps) {
  ...
  function renderPreloadAudio(index: number) {
    const item = broadcast.items[index]
    if (!item) {
      return null
    }

    return <audio key={item.id} preload="auto" src={item.publicUrl} className="hidden" aria-hidden="true" />
  }
  ...
  return (
    <Section>
      <Section.Header title={broadcast.title} description={broadcast.description || "Audio broadcast"} />
      <Section.Body className="gap-4">
        ...
        {preloadIndices.map(renderPreloadAudio)}
      </Section.Body>
    </Section>
  )
}
